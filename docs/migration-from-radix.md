...
| `Tooltip`, `HoverCard` | `delayDuration`/`openDelay` unchanged. The trigger may be an `<a>`, which is what makes link previews work without a wrapper. A long press opens both on a touch screen, where Radix opens neither — see [touch](./tooltip.html#touch). |
