...
| **The saving is uneven** | Where the platform hands over a whole element, almost nothing ships. Where it hands over only the layering, roving focus and typeahead still ship, and menus keep the largest bundle in the library. |
...
| 3 | [The saving is uneven](#3-the-saving-is-uneven) | You are budgeting from the Select or Dialog headline |
...
| DropdownMenu, ContextMenu, Menubar, NavigationMenu, Tabs, Toolbar, ToggleGroup | layering only, and `roving.ts` ships from either entry point | DropdownMenu 31.6 → 3.55 kB |

Budget from this table rather than from the Select headline, and read it by
group rather than by size. Menus are the case people get backwards: they save
more kilobytes than anything except Select, and they are also the largest thing
bedrock ships, because roving focus and typeahead have to come from somewhere.
`/controlled` buys almost nothing for that last group either.
...
<Dialog.Trigger asChild>
  <Link href="/upgrade">Upgrade</Link>   {/* renders <a>, so it throws */}
</Dialog.Trigger>
```
...
`<CheckIcon />` inside a checkbox has nowhere to go, and becomes a masked
`::before` instead. A spinner in an overlay, a scrollbar with its own hover animation,
...
| --- | --- | --- | --- |
| 7 | No light dismiss by default | A modal `<dialog>` does not close on a backdrop click. Users report this as "the dialog is stuck". | `closedby="any"` restores it per dialog. There is no prop yet, because the rule against platform names in the public API rules out `closedby` and Radix has no name to copy. |
...
| 10 | It is young | `0.1.1`, one contributor, no deprecation policy. Several questions on this page have answers that would change public API. | Adopting now means being the one who finds them. |
