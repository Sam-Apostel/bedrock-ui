...
possible. Whether it is a good idea is
[a separate question](./should-you-switch.md), and worth settling first: what
changes is your test suite and your CSS conventions, not your component tree.
...
| `forceMount` | delete it, and check what depended on it | Closed content unmounts, as in Radix, but there is no way to opt out. See [should you switch?](./should-you-switch.md#710-the-smaller-ones). |
...
Check the results by hand where the selector targeted an *ancestor*, which the
`:has()` note in [styling](./styling.md#state-selectors) covers.
...
If you genuinely cannot change the element, say a third-party component that
...
your `Close` button are the ways out. Nothing about focus or dismissal is
broken. It is one fewer way to close.

`<dialog closedby="any">` restores it and will be an opt-in prop once the naming
question in [should you switch?](./should-you-switch.md#710-the-smaller-ones) is settled. If
...
| **`onEscapeKeyDown`, `onPointerDownOutside`, `onInteractOutside`, `onOpenAutoFocus`** | Props on `Dialog.Content`. | None exist. Escape is `cancel`, refusable through `onOpenChange` under the controlled root. There is no outside-pointer event because there is no light dismiss. | For the common `onOpenAutoFocus` case, focusing a specific field rather than the first tabbable, put `autofocus` on that element and `showModal()` honours it. |
...
Radix codebase. It is written for the agent rather than for you, and it is still worth
reading: it is the shortest honest description of what a migration actually
involves.
